...
import { exportToWord } from '../services/exportServiceThuaKe';
import { useReactToPrint } from 'react-to-print';
import toast from 'react-hot-toast';

export default function TabChuyenQuyenThuaKe() {
    ...
    <div className="flex flex-col h-full bg-transparent overflow-y-auto p-6">
      <div className="max-w-6xl mx-auto w-full space-y-6">
        ...
        <div className="bg-white/90 backdrop-blur-md p-6 rounded-2xl shadow-sm border border-slate-200/60">
          <h1 className="text-2xl font-semibold text-slate-800">Chuyển Do Nhận Thừa Kế</h1>
            ...
            {files.length === 0 ? (
              <div 
                ...
                className={`border-2 border-dashed rounded-2xl p-8 text-center cursor-pointer transition-colors backdrop-blur-md
                  ${isDragActive ? 'border-orange-500 bg-orange-50/90' : 'border-slate-300 hover:border-orange-400 bg-white/90'}`}
              >
                <input {...getInputProps()} />
                ...
                <div className="p-4 border-b border-slate-100 flex flex-col gap-3 bg-slate-50/50">
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                      <label className="cursor-pointer px-3 py-1.5 text-sm font-medium text-orange-600 bg-orange-50 hover:bg-orange-100 rounded-lg transition-colors">
                        + Thêm file
                  ...
                  <div className="pt-2">
                    <button 
                      ...
                      className="w-full flex items-center justify-center py-2.5 px-4 bg-orange-600 hover:bg-orange-700 text-white rounded-lg font-medium transition-colors disabled:opacity-70 disabled:cursor-not-allowed"
                    >
                      {isExtracting ? (
                        <>
              ...
              <div className="bg-white/90 backdrop-blur-md rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden">
                <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                  <h3 className="font-semibold text-slate-800 flex items-center">
                    <Edit3 size={18} className="mr-2 text-orange-500" />
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      ...
                      <div>
                        ...
                        <input type="text" name="processingDate" value={extractedData.processingDate || ''} onChange={handleInputChange} className="w-full text-sm p-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" placeholder="VD: 15/08/2026" />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="pt-3 border-t border-slate-100 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input type="text" name="gcnNumber" value={extractedData.gcnNumber || ''} onChange={handleInputChange} className="w-full text-sm p-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs text-slate-500 mb-1">Ngày cấp GCN</label>
                        <input type="text" name="gcnDate" value={extractedData.gcnDate || ''} onChange={handleInputChange} className="w-full text-sm p-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs text-slate-500 mb-1">Thửa đất số</label>
                        <input type="text" name="parcelNumber" value={extractedData.parcelNumber || ''} onChange={handleInputChange} className="w-full text-sm p-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" />
                      </div>
                      <div>
                        <label className="block text-xs text-slate-500 mb-1">Tờ bản đồ số</label>
                        <input type="text" name="mapSheetNumber" value={extractedData.mapSheetNumber || ''} onChange={handleInputChange} className="w-full text-sm p-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" />
                      </div>
                      <div className="col-span-2">
                        <label className="block text-xs text-slate-500 mb-1">Địa chỉ thửa đất</label>
                        <input type="text" name="landAddress" value={extractedData.landAddress || ''} onChange={handleInputChange} className="w-full text-sm p-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" />
                      </div>
                      ...
                      <div className="col-span-2">
                        ...
                        <input type="text" name="usageForm" value={extractedData.usageForm || ''} onChange={handleInputChange} className="w-full text-sm p-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="pt-3 border-t border-slate-100 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input type="text" name="residentialDuration" value={extractedData.residentialDuration || ''} onChange={handleInputChange} className="w-full text-sm p-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" />
                      </div>
                      <div>
                      ...
                      <div className="col-span-2">
                        ...
                        <textarea name="residentialOrigin" value={extractedData.residentialOrigin || ''} onChange={handleInputChange as any} className="w-full text-sm p-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" rows={2} />
                      </div>
                      <div className="col-span-2">
                        <label className="block text-xs text-slate-500 mb-1">Nguồn gốc sử dụng đất nông nghiệp</label>
                        <textarea name="agriculturalOrigin" value={extractedData.agriculturalOrigin || ''} onChange={handleInputChange as any} className="w-full text-sm p-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" rows={2} />
                      </div>
                      <div className="col-span-2">
                        <label className="block text-xs text-slate-500 mb-1">Ghi chú</label>
                        <textarea name="notes" value={extractedData.notes || ''} onChange={handleInputChange as any} className="w-full text-sm p-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" rows={3} />
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <div className="hidden xl:flex items-center space-x-2 border-r border-slate-200 pr-4">
                    ...
                    <select 
                      ...
                      className="text-sm border border-slate-300 rounded-md px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-orange-500 bg-white"
                    >
                  ...
                  <div className="hidden 2xl:flex items-center space-x-2 border-r border-slate-200 pr-4">
                    ...
                    <div className="flex items-center gap-1">
                      <input type="number" value={marginTop} onChange={e => setMarginTop(Number(e.target.value))} className="w-12 text-sm border border-slate-300 rounded-md px-1 py-1 focus:outline-none focus:ring-1 focus:ring-orange-500 text-center" title="Lề trên" />
                      <input type="number" value={marginBottom} onChange={e => setMarginBottom(Number(e.target.value))} className="w-12 text-sm border border-slate-300 rounded-md px-1 py-1 focus:outline-none focus:ring-1 focus:ring-orange-500 text-center" title="Lề dưới" />
                      <input type="number" value={marginLeft} onChange={e => setMarginLeft(Number(e.target.value))} className="w-12 text-sm border border-slate-300 rounded-md px-1 py-1 focus:outline-none focus:ring-1 focus:ring-orange-500 text-center" title="Lề trái" />
                      <input type="number" value={marginRight} onChange={e => setMarginRight(Number(e.target.value))} className="w-12 text-sm border border-slate-300 rounded-md px-1 py-1 focus:outline-none focus:ring-1 focus:ring-orange-500 text-center" title="Lề phải" />
                    </div>
                  </div>
              ...
              <div className="flex-1 p-4 sm:p-8 bg-slate-100/50 overflow-y-auto flex justify-center items-start">
                ...
                >
                  <div className="flex justify-between mb-6 items-start">
                    ...
                    <div className="text-center flex flex-col items-center">
                      ...
                      <p className="italic mt-2 whitespace-nowrap" style={{ fontSize: `${previewFontSize}pt` }}>
                        Hà Tĩnh, {extractedData?.processingDate || "    /      / 2026"}
                      </p>
                    </div>
                  </div>
                  ...
                  <p className="mb-8 text-justify indent-8">
                    Hồ sơ đủ điều kiện chuyển quyền sử dụng đất, quyền sở hữu nhà ở và tài sản khác gắn liền với đất theo Điều 45 Luật Đất đai năm 2024 và các quy định khác của pháp luật. Kính đề nghị Văn phòng đăng ký đất đai tỉnh Hà Tĩnh thẩm tra hồ sơ, trình ký cấp Giấy chứng nhận QSD đất, quyền sở hữu nhà ở và tài sản khác gắn liền với đất cho <span className="font-semibold">{extractedData?.buyerName || "bà Nguyễn Thị Luyn"}</span> theo quy định pháp luật./.
                  </p>
                  ...
                  <div className="flex justify-between text-center mt-8 break-inside-avoid">
                    <div className="w-1/2 flex flex-col items-center">
                      <p className="italic">Hà Tĩnh, {extractedData?.processingDate || "    /      / 2026"}</p>
